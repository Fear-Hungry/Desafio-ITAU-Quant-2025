...
def test_inverse_variance_matches_expected_on_diagonal():
    cov = pd.DataFrame(np.diag([0.04, 0.09, 0.16]), index=["A", "B", "C"], columns=["A", "B", "C"])
...
def test_heuristic_allocation_dispatches_methods():
    cov = pd.DataFrame(np.diag([0.04, 0.09]), index=["AAA", "BBB"], columns=["AAA", "BBB"])
    returns = pd.DataFrame(
        np.random.default_rng(1).normal(size=(60, 3)),
        columns=["X", "Y", "Z"],
    )

    result_eq = hrp_mod.heuristic_allocation({"assets": ["AAA", "BBB"]}, method="equal_weight")
    assert result_eq.weights.index.tolist() == ["AAA", "BBB"]
    assert result_eq.diagnostics == {"count": 2}

    result_iv = hrp_mod.heuristic_allocation({"covariance": cov}, method="inverse_variance")
    ...
    result_cluster = hrp_mod.heuristic_allocation({"returns": returns}, method="cluster", config={"n_clusters": 2})
