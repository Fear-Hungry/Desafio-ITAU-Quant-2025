...
def test_group_lasso_penalty_handles_named_groups():
    ...
    penalty = penalties_mod.group_lasso_penalty(weights, groups, gamma=0.3, asset_index=["AAPL", "MSFT", "IEF", "TLT"])
...
def test_turnover_penalty_uses_previous_weights():
    ...
    penalty = penalties_mod.turnover_penalty(weights, prev, gamma=0.7, asset_index=["AAA", "BBB"], normalised=True)
    assert penalty.is_convex()
...
def test_penalty_factory_combines_terms():
    ...
    penalties = penalties_mod.penalty_factory(weights, config, asset_index=["A", "B", "C"])
...
def test_cardinality_penalty_rejects_invalid_method():
    ...
    with pytest.raises(ValueError):
        penalties_mod.cardinality_soft_penalty(weights, k_target=1, gamma=0.5, method="invalid")
