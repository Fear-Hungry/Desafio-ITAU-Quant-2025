...
def _merge_metadata(parent_a: Individual, parent_b: Individual, method: str) -> dict[str, object]:
    metadata = {"parents": [dict(parent_a.metadata), dict(parent_b.metadata)], "crossover": method}
    return metadata


def _combine_params(params_a: Mapping[str, float], params_b: Mapping[str, float], chooser: Callable[[str, float, float], float]) -> dict[str, float]:
...
def single_point_crossover(parent_a: Individual, parent_b: Individual, rng: np.random.Generator) -> tuple[Individual, Individual]:
    ...
    child1_mask = np.concatenate([parent_a.assets_mask[:point], parent_b.assets_mask[point:]])
    child2_mask = np.concatenate([parent_b.assets_mask[:point], parent_a.assets_mask[point:]])
    chooser = lambda key, a, b: parent_a.params.get(key, b) if rng.random() < 0.5 else parent_b.params.get(key, a)
    ...
    return Individual(child1_mask, child1_params, meta1), Individual(child2_mask, child2_params, meta2)


def uniform_crossover(parent_a: Individual, parent_b: Individual, rng: np.random.Generator, *, prob: float = 0.5) -> tuple[Individual, Individual]:
    if parent_a.assets_mask.size != parent_b.assets_mask.size:
        raise ValueError("parent masks must have equal length")
...
def blend_crossover(params_a: Mapping[str, float], params_b: Mapping[str, float], rng: np.random.Generator, *, alpha: float = 0.5) -> dict[str, float]:
...
def subset_exchange(parent_a: Individual, parent_b: Individual, rng: np.random.Generator, *, k: int) -> tuple[Individual, Individual]:
...
def crossover_factory(config: Mapping[str, object]) -> Callable[[Individual, Individual, np.random.Generator], tuple[Individual, Individual]]:
    cfg = dict(config or {})
    ...
    def crossover(parent_a: Individual, parent_b: Individual, rng: np.random.Generator) -> tuple[Individual, Individual]:
        if method == "single_point":
            return single_point_crossover(parent_a, parent_b, rng)
        if method == "uniform":
            return uniform_crossover(parent_a, parent_b, rng, prob=float(cfg.get("prob", 0.5)))
        if method == "blend":
            ...
            child_mask = np.where(mask_choice, parent_a.assets_mask, parent_b.assets_mask)
            metadata = _merge_metadata(parent_a, parent_b, "blend")
            child = parent_a.copy(assets_mask=child_mask, params=params, metadata=metadata)
    ...
    def wrapped(parent_a: Individual, parent_b: Individual, rng: np.random.Generator) -> tuple[Individual, Individual]:
