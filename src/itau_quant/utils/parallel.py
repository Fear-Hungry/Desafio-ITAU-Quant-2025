    ...
    if backend in executor_map:
        ...
        with executor_map[backend](max_workers=max_workers) as executor:
            ...
            try:
                ...
                logging.error(
                    f"Job '{job_name}' excedeu o timeout global de {timeout}s."
                )
                raise
        ...
        raise ValueError(
            f"Backend '{backend}' não reconhecido. Use 'process', 'thread', 'joblib' ou 'sequential'."
        )
...
def collect_exceptions(results: List[Any], re_raise: bool = True):
    ...
    if exceptions and re_raise:
        ...
        raise RuntimeError(
            f"{len(exceptions)} worker(s) falharam com as seguintes exceções:\n{error_messages}"
        )
...
def parallel_backtest(
    ...
    results_with_errors = parallel_map(
        backtest_engine, strategies, backend, max_workers
    )
