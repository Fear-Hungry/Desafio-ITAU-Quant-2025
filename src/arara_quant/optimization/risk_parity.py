"""Risk parity helpers and solvers.

This module centralises the risk-parity implementations used across the codebase.
The legacy entry-point ``arara_quant.optimization.core.risk_parity`` remains as a
thin wrapper to preserve backwards compatibility.
"""
...
def normalise_long_only_weights(
    weights: Sequence[float] | np.ndarray,
    *,
    clip_lower: float = 0.0,
) -> np.ndarray:
    """Normalise a long-only weights vector to sum to 1."""

    array = np.asarray(weights, dtype=float).ravel()
    if array.ndim != 1:
        raise ValueError("weights must be 1-dimensional")
    if clip_lower is not None:
        array = np.clip(array, float(clip_lower), None)
...
def _solve_calibrated_erc(
    cov: pd.DataFrame,
    *,
    init_weights: Sequence[float] | pd.Series | None,
    config: Mapping[str, object],
) -> pd.Series:
    """Solve the CVXPY-based calibrated ERC variant (risk parity with penalties)."""
    erc = importlib.import_module("arara_quant.optimization.erc_calibrated")
    calibrate_eta_for_turnover = erc.calibrate_eta_for_turnover
    calibrate_gamma_for_vol = erc.calibrate_gamma_for_vol
    solve_erc_core = erc.solve_erc_core
    solve_erc_with_cardinality = erc.solve_erc_with_cardinality
    ...
    prev = config.get("previous_weights", init_weights)
    if prev is None:
        prev_vector = np.full(n_assets, 1.0 / n_assets, dtype=float)
    elif isinstance(prev, pd.Series):
        prev_series = prev.reindex(assets).fillna(0.0).astype(float).clip(lower=0.0)
        total = float(prev_series.sum())
        prev_vector = (
            prev_series.to_numpy(dtype=float) / total
            if total > 0 and np.isfinite(total)
            else np.full(n_assets, 1.0 / n_assets, dtype=float)
        )
    else:
        prev_vector = normalise_long_only_weights(prev)

    costs_raw = (
        config.get("costs")
        or config.get("transaction_costs")
        or config.get("cost_vector")
    ...
    gamma_raw = config.get("gamma")
    if gamma_raw is None:
        vol_target = config.get("vol_target", config.get("volatility_target"))
        if vol_target is None:
            raise ValueError("calibrated method requires 'gamma' or 'vol_target'")
        eta_seed = float(config.get("eta", 0.0))
        _, gamma, _ = calibrate_gamma_for_vol(
            cov_matrix,
            prev_vector,
            eta=eta_seed,
            costs=cost_vector,
            ...
            vol_tolerance=float(config.get("vol_tolerance", 0.01)),
            max_iter=int(config.get("gamma_max_iter", 25)),
            verbose=verbose,
        )
    else:
        gamma = float(gamma_raw)

    eta_raw = config.get("eta")
    if eta_raw is None:
        turnover_target = config.get("turnover_target", config.get("target_turnover"))
        if turnover_target is None:
            raise ValueError("calibrated method requires 'eta' or 'turnover_target'")
        _, eta, _ = calibrate_eta_for_turnover(
            cov_matrix,
            prev_vector,
            gamma=gamma,
            costs=cost_vector,
            ...
            groups=groups,
            asset_names=assets,
            target_turnover=float(turnover_target),
            tolerance=float(config.get("turnover_tolerance", 0.01)),
            max_iter=int(config.get("eta_max_iter", 20)),
            verbose=verbose,
        )
    else:
        eta = float(eta_raw)

    k_raw = config.get("cardinality", config.get("K", config.get("top_k")))
    if k_raw is not None:
        weights, _ = solve_erc_with_cardinality(
            cov_matrix,
            prev_vector,
            gamma=gamma,
            eta=eta,
...
def risk_parity(
    ...
    *,
    ...
    try:
        ...
        elif method in {"calibrated", "erc", "erc_calibrated"}:
            weights = _solve_calibrated_erc(
                cov_df, init_weights=init_weights, config=config
            )
        else:
