...
from arara_quant.optimization.solvers import resolve_config_path
...
def _run_simulation(
    ...
    def _drift_pretrade(prev_w: pd.Series, seg_returns: pd.DataFrame) -> pd.Series:
        """Drift previous weights with realized returns to obtain pre-trade weights.

        prev_w: weights at last rebalance (post-trade, fractions summing to 1)
        seg_returns: returns between last rebalance (exclusive) and current date (inclusive)
        """
        if prev_w.empty:
            return prev_w
        if seg_returns is None or seg_returns.empty:
            return prev_w
        growth = (1.0 + seg_returns).prod(axis=0)
        growth = growth.reindex(prev_w.index).fillna(1.0)
        w_pre = prev_w * growth
        total = float(w_pre.sum())
    ...
    for split in splits:
        ...
        # Compute pre-trade weights by drifting previous_weights with realized returns
        if last_rebalance_date is None:
            prev_pretrade = previous_weights
        else:
            seg = returns.loc[(returns.index > last_rebalance_date) & (returns.index <= first_day)]
            prev_pretrade = _drift_pretrade(previous_weights, seg)

        rebalance_result = rebalance(
            date=first_day,
            market_data=market,
            previous_weights=prev_pretrade,
