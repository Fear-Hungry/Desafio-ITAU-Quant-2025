    ...
    if isinstance(obj, (pd.DataFrame, pd.Series)):
        if obj.isnull().values.any():  # essa propriedade retorna True se houver NaNs
            ...
            if isinstance(obj, pd.DataFrame):  # DataFrame
                ...
                details = ", ".join(
                    [f"(row {r}, col '{obj.columns[c]}')" for r, c in nan_positions]
                )
            else:  # Series
                nan_positions = obj[obj.isnull()].index.tolist()  # índices dos NaNs
                details = ", ".join(
                    [f"index {i}" for i in nan_positions]
        ...
        if np.isnan(obj).any():  # verifica se há NaNs
            ...
            details = ", ".join(
                [f"index {tuple(idx)}" for idx in nan_indices]
            )  # formatar para mensagem
...
def assert_symmetric(matrix, atol=1e-8):
    ...
    if not np.allclose(matrix, matrix.T, atol=atol):  # compara com a transposta
        raise ValueError(
            "A matriz não é simétrica dentro da tolerância especificada."
        )  # não é simétrica
...
def assert_psd(matrix, atol=1e-8):
    ...
    eigenvalues = np.linalg.eigvalsh(
        matrix
    )  # eigvalsh é mais estável para matrizes simétricas
    if np.any(eigenvalues < -atol):  # verifica se algum autovalor é menor que -atol
        raise ValueError(
            "A matriz não é semidefinida positiva dentro da tolerância especificada."
        )  # não é PSD
...
def validate_returns_frame(df):
    ...
    if not pd.api.types.is_datetime64_any_dtype(
        ...
        raise TypeError(
            "O índice do DataFrame deve ser do tipo datetime."
        )  # índice inválido
    if not df.index.is_monotonic_increasing:  # verifica se o índice está ordenado
        raise ValueError(
            "O índice do DataFrame deve estar ordenado em ordem crescente."
        )  # não está ordenado
    ...
    if not all(
        ...
        raise TypeError(
            "Todas as colunas do DataFrame devem ser de tipo numérico."
...
def validate_weights_vector(weights):
    ...
    if not np.isclose(total, 1.0, atol=1e-6):  # verifica se a soma é aproximadamente 1
        raise ValueError(
            f"A soma dos pesos deve ser 1.0, mas é {total}."
        )  # soma incorreta
    if np.any(
        np.abs(weights) > 1e6
    ):  # verifica se algum peso é muito grande em magnitude
        raise ValueError(
            "Alguns pesos têm magnitude excessivamente alta."
        )  # magnitude alta
