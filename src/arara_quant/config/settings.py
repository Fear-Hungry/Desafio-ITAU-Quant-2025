...
ENV_PREFIX = "ARARA_QUANT_"
LEGACY_ENV_PREFIXES: tuple[str, ...] = ()
"""Prefixos utilizados para todas as variáveis de ambiente do projeto."""


def _env_keys(name: str) -> list[str]:
    return [f"{ENV_PREFIX}{name}", *[f"{legacy}{name}" for legacy in LEGACY_ENV_PREFIXES]]


def _lookup_env(mapping: Mapping[str, str], name: str) -> str | None:
    ...
    def from_env(
        ...
        *,
        ...
        if project_root_value is None:
            project_root_value = _lookup_env(env_mapping, "PROJECT_ROOT")
        if project_root_value is None:
            project_root_value = _lookup_env(system_environ, "PROJECT_ROOT")
        ...
        def pull(name: str, *, default: Any, target: type) -> Any:
            if name in overrides:
                ...
                return value
            raw = _lookup_env(env_mapping, name)
            if raw is not None:
                if target is Path:
