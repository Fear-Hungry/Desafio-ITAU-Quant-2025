...
- **Master Orchestrator** (`arara_quant.runners.validation.run_master_validation`) - Coordinates 7-stage validation pipeline
- **Unified CLI** (`arara-quant`) - 11 commands for backtesting, comparison, optimization
- **Research Runners** (15 modules) - Sensitivity analyses and experimentation
- **Validation Suite** (4 modules) - Constraint and estimator validation
...
poetry run python -m arara_quant.runners.validation.run_master_validation --mode production
```
...
poetry run arara-quant run-full-pipeline --config configs/optimization/optimizer_example.yaml
```
...
```bash
poetry run python -m arara_quant.runners.validation.run_master_validation \
    --mode quick \
    --skip-download
```
...
- Stage 5: Validation suite (4 test modules)
- Stage 6: Results aggregation
- Stage 7: Report generation
...
| Mode | Configs |
|------|---------|
| `quick` | `configs/optimization/optimizer_example.yaml` |
| `full` | `configs/optimization/optimizer_example.yaml`<br>`configs/optimization/optimizer_regime_aware.yaml`<br>`configs/optimization/optimizer_adaptive_hedge.yaml` |
...
**Command (example):**
```bash
poetry run python -m arara_quant.runners.research.run_cost_sensitivity
```
...
**Command (example):**
```bash
poetry run python -m arara_quant.runners.validation.run_comprehensive_tests
```
...
poetry run python -m arara_quant.runners.core.run_01_data_pipeline --force-download
```
...
1. **Check estimation window:**
   ```bash
   poetry run python -m arara_quant.runners.research.run_window_sensitivity
   ```
...
4. **Verify regime detection:**
   ```bash
   poetry run python -m arara_quant.runners.research.run_regime_stress
   ```
...
**Diagnostic:**
```bash
poetry run python -m arara_quant.runners.validation.run_constraint_tests
```
...
3. **Group constraints:** Group allocation > limit
   - Review `configs/universe/asset_groups.yaml`
...
1. **Create runner in `src/arara_quant/runners/research/`:**
   ```python
   # arara_quant.runners.research.run_my_experiment
...
2. **Add to orchestrator:**
   ```python
   # In arara_quant.runners.validation.run_master_validation
...
- Consult `docs/guides/QUICKSTART.md` for basic usage
