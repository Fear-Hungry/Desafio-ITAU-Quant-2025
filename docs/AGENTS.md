...
- Core library lives under `src/arara_quant/` with thematic packages (`backtesting/`, `optimization/`, `risk/`, `utils/`) mirroring the investment workflow.
...
- Locate reusable config keys in `arara_quant.config`; prefer `Path` objects over plain strings when handling filesystem paths.
...
- For stochastic components, seed via `arara_quant.utils.random` helpers to ensure deterministic runs.
...
- Validate new YAML configs with `poetry run arara-quant show-settings --json` before publishing to avoid runtime schema regressions.
